...
`gmr` (Git Merge Request) is a single-file Bash script that automates the merge request / pull request workflow. It stages changes, generates a commit message via AI (Gemini / Codex), creates a branch, commits, and opens a GitLab MR or GitHub PR — all in one command. Platform is auto-detected from the `origin` remote URL.
...
- `GMR_ANTHROPIC_MODEL` — Anthropic model (default: `Codex-sonnet-4-20250514`)
...
2. `git add -A` + generate commit message: Gemini → Codex → OpenAI → manual input
